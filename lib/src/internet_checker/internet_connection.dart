...
import 'package:connected/src/internet_checker/request_manager/request_manager.dart';
import 'package:connectivity_plus/connectivity_plus.dart';
import 'package:dio/dio.dart';

import 'dio_interceptor/timeout_interceptor.dart';
import 'internet_check_option.dart';
import 'internet_check_result.dart';
...
class InternetConnection {
  ...
  InternetConnection.createInstance(
      ...
      this.connectionTimeout = const Duration(seconds: Configs.timeOut)})
      : assert(
          useDefaultOptions ||
    ...
    _internetCheckOptions = [
      ...
      if (customCheckOptions != null)
        ...customCheckOptions.map(
          (option) => option.copyWith(
              timeout: connectionTimeout), // Pass connectionTimeout here
        ),
    ];
  ...
  Future<InternetCheckResult> _checkReachabilityFor(
    ...
    try {
      ...
      return InternetCheckResult(
          option: option, isSuccess: response?.statusCode == 200);
    } catch (_) {
      return InternetCheckResult(
        option: option,
        isSuccess: false,
